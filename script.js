// Le's Auto Repair - Interactive Scripts
...
const statsObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (suffix) {
                // Non-numeric values like "Family" or "Cert."
                fadeInElement(el);
            } else if (count !== null) {
...
contactForm.addEventListener('submit', function(e) {
    ...
    window.location.href = `mailto:info@lesautorepair.com?subject=${subject}&body=${body}`;
...
document.querySelectorAll('a[href^="tel:"]').forEach(link => {
    link.addEventListener('click', () => {
        if (typeof gtag === 'function') {
            gtag('event', 'click_to_call', { business: "Le's Auto Repair" });
        }
    });
});
